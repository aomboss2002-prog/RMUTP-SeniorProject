...
function versioned_asset_url(string $path): string
{
    ...
    if (!isset($versions[$relativePath])) {
        // Keep unchanged assets cached across deployments, while CLI deploys
        // of uncommitted edits must also receive a fresh cache key.
        $file = __DIR__ . '/../assets/' . $relativePath;
        $contentHash = is_file($file) ? hash_file('sha256', $file) : false;
        $versions[$relativePath] = $contentHash !== false ? substr($contentHash, 0, 16) : '1';
    }
