...
## ตรวจเวลาโหลดแต่ละหน้า

เปิด `login.php?perf=1` แล้วเข้าสู่ระบบตามบทบาทและไล่เปิดหน้าที่ต้องการตรวจในแท็บเดิม โหมดตรวจจะติดตามข้ามหน้าโดยอัตโนมัติ ให้รอประมาณ 15 วินาทีต่อหน้าเพื่อเก็บตัวอย่าง API ที่ทำงานหลังเปิดหน้า

- แสดง TTFB (เวลารอข้อมูลแรก), DOM Ready, Load, LCP และคำขอ API/ทรัพยากรที่ช้าที่สุด
- เก็บ 50 ตัวอย่างล่าสุดใน Session Storage ของแท็บนั้น และส่งออกด้วยปุ่ม **ดาวน์โหลด JSON**
- เปรียบเทียบทั้งครั้งแรกและการเปิดซ้ำ เพราะแคช เครือข่าย และ Serverless cold start มีผลต่อเวลา
- ปิดด้วยปุ่ม **ปิดโหมดตรวจ** หรือเปิดหน้าใดก็ได้พร้อม `?perf=0` โหมดปกติไม่โหลดไฟล์ตัวตรวจ

ค่า Load ไม่ได้หมายความว่า API ทุกตัวเสร็จแล้ว รายงานนับเฉพาะคำขอที่เสร็จในช่วงเก็บตัวอย่าง ขนาดรับส่งข้ามโดเมนอาจอ่านไม่ได้ และแผงตรวจอาจมีผลต่อ LCP จึงไม่ใช่คะแนน Lighthouse ผลเก็บในเบราว์เซอร์เท่านั้น ไม่ส่งไปเซิร์ฟเวอร์และไม่เก็บเนื้อหาตอบกลับหรือ Token ใน Query String เครื่องมือนี้วัดเฉพาะหน้าที่เปิดจริง ไม่เปิดไล่ทุกหน้าเอง และไม่ข้ามสิทธิ์การเข้าสู่ระบบ

## AI บน Vercel โดยไม่ต้องเปิด BAT ค้าง

ระบบ Production จะตรวจชื่อโครงงานทันทีเมื่อบันทึก โดย Vercel ใช้ `local-ngram-v1` ซึ่งไม่ต้องใช้ API Token และไม่พยายามเชื่อมต่อ Ollama ที่อยู่ในเครื่องผู้พัฒนา ส่วนหน้าเว็บอ่าน Risk Score ที่ worker บันทึกไว้เท่านั้น ไม่คำนวณใหม่ระหว่างเปิดหน้า มี Vercel Cron ประมวลผลวันละครั้ง รอบละ 20 โครงงาน เรียงจากคะแนนที่ยังไม่มีหรือเก่าที่สุด หากมีมากกว่า 20 โครงงาน ผลอาจใช้หลายวันจึงอัปเดตครบ ควรใช้ worker ต่อเนื่องหรือปรับตารางเวลาตามข้อจำกัดโฮสติ้งเมื่อใช้งานจริง
...
### การตรวจจุดช้าฝั่งนักศึกษา

GET ไทม์ไลน์และข้อความอ่านเฉพาะชุดข้อมูลที่ใช้จาก Runtime JSON แทน `load_data()` และปล่อย Session lock ก่อนอ่าน ไทม์ไลน์ใช้ประวัติถาวรเป็นหลัก หากไม่มีจึงอ่านเอกสาร/ผลพิจารณาเพื่อสร้างข้อมูลแบบเดิม หน้าไทม์ไลน์และข้อความเรียกตัวนับแจ้งเตือนหลังคำขอหลักเสร็จในครั้งแรก ส่วน POST ส่งข้อความ/บันทึกข้อมูลยังใช้เส้นทางเดิมพร้อม CSRF การปรับนี้ยังไม่ใช่การแบ่งหน้าข้อความ และยังมีต้นทุนการอ่าน JSON ที่ฐานข้อมูล ต้องวัดจริงหลัง Deploy

API รูปโปรไฟล์อ่านเฉพาะ `id, photo` จากตารางนักศึกษา หากดูรูปคนอื่นจะตรวจข้อมูลกลุ่มจาก Runtime JSON เช่นเดิม ไม่ข้ามการตรวจสิทธิ์ ส่วน GET แจ้งเตือนอ่านเฉพาะกลุ่ม/แจ้งเตือนจาก Runtime JSON และตรวจว่าบัญชีนักศึกษายังมีอยู่ ทั้งสองเส้นทางปล่อย Session lock ก่อนงานอ่านข้อมูล/Storage; POST ทำเครื่องหมายอ่านแล้วยังคงใช้เส้นทางบันทึกและ CSRF เดิม

ผล JSON จากเครื่องมือตรวจความเร็วเพิ่ม `htmlReceivedMs` (รับ HTML ครบ), `htmlReceiveDurationMs` (ตั้งแต่ไบต์แรกถึงรับครบ), `afterHtmlToDomMs` (หลังรับครบถึง DOMContentLoaded) และเวลาเริ่ม/สิ้นสุดของ Resource ค่าหลัง HTML ไม่ใช่เวลา JavaScript ล้วน และไม่ได้วัดความพร้อมข้อมูลครบทั้งหน้า การอ่าน JSON ยังมีต้นทุนฝั่งฐานข้อมูล ต้องวัดบน Production ก่อนยืนยันว่าระยะเวลาลดลง

ทดสอบการอ่านและขอบเขตแจ้งเตือนแบบไม่แตะฐานข้อมูลจริง: `C:\xampp\php\php.exe tests\verify-runtime-read.php`

### การโหลดรายชื่อนักศึกษา Admin

Dashboard ไม่ส่งรายชื่อนักศึกษาและโครงงานทั้งหมดพร้อมข้อมูลเริ่มต้น ช่องค้นหาจะรอหยุดพิมพ์ 300 ms แล้วเรียก API เฉพาะ Admin และคืนผลสูงสุดประเภทละ 4 รายการ โดยไม่ Cache ข้อมูลส่วนตัว

Admin Dashboard อ่านยอดนักศึกษา/อาจารย์และสถิติโครงงาน/เอกสารด้วย SQL aggregate โดยไม่เรียก `load_data()` เอกสารล่าสุดใช้ `ORDER BY uploaded_at DESC, id DESC LIMIT 5` ส่วนกิจกรรม การแจ้งเตือน และผลพิจารณายังใช้ Runtime JSON เป็นแหล่งหลัก จึงเลือกเฉพาะ 5 รายการแรกด้วย `JSON_EXTRACT` เพื่อรักษาลำดับเดิม ไม่อ่านตารางที่อาจยังไม่ได้ Sync และไม่ย้ายหรือลบข้อมูล การอ่าน JSON ฝั่งฐานข้อมูลยังมีต้นทุน จึงยังไม่ถือว่าย้ายออกจาก JSON สมบูรณ์

Endpoint นี้ใช้ 4 คำสั่งอ่านต่อคำขอ ไม่มี Query ในลูปหรือ Cache ร่วมระหว่างผู้ใช้ การลดปริมาณข้อมูลไม่ได้รับประกันว่าจะเร็วขึ้นทุกสภาพเครือข่าย ต้องวัดเวลา MySQL/Production ก่อนสรุป ตรวจเบื้องต้นด้วย `C:\xampp\php\php.exe tests\verify-admin-dashboard.php` (SQLite แยกจากข้อมูลจริง)

ตารางโครงงานและเอกสาร (รวมหน้าข้อเสนอ/Draft/Complete ของ Admin) ใช้ `action=page` ค้นหา กรอง เรียง และแบ่งหน้าที่ฐานข้อมูล เริ่มต้น 25 รายการ สูงสุด 50 รายการต่อคำขอ ไม่โหลด Runtime state ทั้งระบบ ความคืบหน้าอ่านค่าที่ระบบคำนวณและบันทึกเมื่อเปลี่ยน Workflow ส่วนสิทธิ์ทำเครื่องหมายเสร็จสมบูรณ์ยังตรวจ Complete ล่าสุดทั้งตอนแสดงและตอนบันทึก ยอดรวมประเภทเอกสารนับจากทั้งหมด ไม่ใช่เฉพาะหน้าปัจจุบัน ปุ่มส่งออกระบุ `(หน้านี้)` และมี `CSV ทั้งหมด` สำหรับส่งออกทุกแถวของประเภทเอกสารที่เปิด (ไม่จำกัดด้วยคำค้น/ตัวกรองตาราง) การส่งออกและรายการเลือกในฟอร์มยังโหลดข้อมูลทั้งหมดเมื่อใช้งาน

ตรวจ SQL แบบแยกจากข้อมูลจริงด้วย `C:\xampp\php\php.exe tests\verify-admin-pagination.php` ใช้ SQLite ในหน่วยความจำ ต้องตรวจ MySQL และ Browser หลัง Login ก่อน Deploy

ตารางนักศึกษาใช้ API `students&action=page` เพื่อค้นหา กรอง และเรียงข้อมูลที่ฐานข้อมูล โหลดเริ่มต้น 25 รายการและจำกัดสูงสุด 50 รายการต่อคำขอ โดยตรวจสิทธิ์ Admin และไม่โหลด Runtime state ทั้งระบบ ปุ่มส่งออกบนหัวหน้าส่งออกทั้งหมด ส่วนปุ่มในตารางส่งออกเฉพาะหน้าที่แสดง ไม่มีการ Cache ข้อมูลส่วนตัวร่วมกันระหว่างผู้ใช้

ตรวจการแบ่งหน้าแบบแยกจากฐานข้อมูลจริงด้วย `C:\xampp\php\php.exe tests\verify-student-pagination.php` (SQLite ในหน่วยความจำ) ก่อนนำขึ้น Production ต้องทดสอบกับ MySQL และบัญชี Admin อีกครั้ง
...
API คลังไม่เรียก `load_data()` ทั้งระบบ: อ่านเฉพาะชุดนักศึกษา/กลุ่ม/โครงงาน/การตั้งค่าจาก Runtime JSON และเลือกเอกสาร Complete ที่อนุมัติจากตาราง SQL การตรวจไฟล์จริงทำหลังค้นหาและแบ่งหน้า จึงไม่เกิน 5 ครั้งต่อคำขอ (อาจเป็น Blob HEAD request เมื่อใช้ Vercel) ยังคงแสดงรายการที่ไฟล์หายโดยไม่มีลิงก์ดาวน์โหลด

Response มี `Server-Timing` แยก `db_connect`, `catalog_read`, `catalog_build` โดยช่วง build รวมการตรวจ Storage ไม่ใช่เวลา CPU ล้วน ตัววัดหน้าเว็บเก็บค่าเหล่านี้ใน JSON ด้วย ผลจาก Cache อาจเป็น timing ของคำขอเดิม การกรอง/แบ่งหน้าคลังยังทำใน PHP และฐานข้อมูลยังต้องประมวลผล JSON จึงยังไม่ใช่ SQL pagination เต็มรูปแบบ ไม่มีการเพิ่ม Index หรือย้าย Region โดยอัตโนมัติ

ทดสอบการจำกัดการตรวจไฟล์และความตรงกันของผลลัพธ์: `C:\xampp\php\php.exe tests\verify-catalog-performance.php` ใช้ SQLite และ Storage จำลอง ไม่เปลี่ยนฐานข้อมูลจริง ส่วน `tests/public-catalog.php` ต้องมี PDF fixture `uploads/complete/complete-prj005.pdf`
